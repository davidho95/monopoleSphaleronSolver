...
#include <ctime>
#include <iomanip>
#include "ElectroweakFieldTools.hpp"
...
namespace monsta
{
  ...
  public:
    ...
    bool solve(monsta::ElectroweakTheory &theory, LATfield2::Field< std::complex<double> > &field, LATfield2::Field< std::complex<double> > &referenceField);
  ...
  void GradDescentSolverChigusa::iterate(LATfield2::Field< std::complex<double> > &field, monsta::ElectroweakTheory &theory, LATfield2::Field< std::complex<double> > &referenceField)
  {
    clock_t begin = clock();
    ...
    for (site.first(); site.test(); site.next())
    {
      for (int matIdx = 0; matIdx < numMatrices; matIdx++)
      {
        ...
        else
        {
          ...
          {
            projMat(jj) = projMat(jj) - real(projMat(jj)*conj(fieldMat(jj)))*fieldMat(jj);
